...
inline static void ggml_vec_pow_f32 (const int n, float * z, const float * x, const float * y) { for (int i = 0; i < n; ++i) z[i]  = powf(x[i], y[i]);   }
...
inline static void ggml_vec_erf_f32 (const int n, float * y, const float * x) { for (int i = 0; i < n; ++i) y[i] = erff(x[i]);  }
...
struct ggml_tensor * ggml_pow_impl(
        struct ggml_context * ctx,
        struct ggml_tensor * a,
        struct ggml_tensor * b,
        bool inplace) {
...
struct ggml_tensor * ggml_pow(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
    return ggml_pow_impl(ctx, a, b, false);
}

struct ggml_tensor * ggml_pow_inplace(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
    return ggml_pow_impl(ctx, a, b, true);
}
...
struct ggml_tensor * ggml_erf_impl(
        struct ggml_context * ctx,
        struct ggml_tensor * a,
        bool inplace) {
    bool is_node = false;
...
struct ggml_tensor * ggml_erf(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_erf_impl(ctx, a, false);
}

struct ggml_tensor * ggml_erf_inplace(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_erf_impl(ctx, a, true);
}
...
static void ggml_compute_forward_pow_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        struct ggml_tensor * dst) {
...
static void ggml_compute_forward_pow(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        struct ggml_tensor * dst) {
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_pow_f32(params, src0, src1, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
// ggml_compute_forward_erf

static void ggml_compute_forward_erf_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    assert(params->ith == 0);
...
static void ggml_compute_forward_erf(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_erf_f32(params, src0, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_POW:
            {
                ggml_compute_forward_pow(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_SQR:
            {
        ...
        case GGML_OP_ERF:
            {
                ggml_compute_forward_erf(params, tensor->src0, tensor);
            } break;
        case GGML_OP_ELU:
            {
